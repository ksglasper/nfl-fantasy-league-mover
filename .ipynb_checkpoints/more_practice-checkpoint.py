...
######################## This section scrapes all H2H data including win/loss, points for/against, and win pecentage against each player for the year data by year ########################
  ...
  for team_num in range(1, len(season_username_master_list) + 1):
    ...
    total_head2head[reg_season_stats_name]["20" + str(year)] = yearly_head2head
    # print(reg_season_stats_name)

    # print(total_head2head[reg_season_stats_name])
...
print(total_head2head["Harsha"]["2022"]["Nolan"]["Win"])
test_df = pd.DataFrame.from_dict({(name, year, opponent): total_head2head[name][year][opponent]
                           for name in total_head2head.keys() 
                           for year in total_head2head[name].keys()
                           for opponent in total_head2head[name][year].keys()},
                       orient='index')
    
test_df.index = pd.MultiIndex.from_tuples(test_df.index)
print(test_df)
